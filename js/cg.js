...
class Mesh {
  ...
  draw(lines = false) {
    ...
    this.gl.drawElements(lines === true ? this.gl.LINE_STRIP : this.gl.TRIANGLES, this.indices.length, this.gl.UNSIGNED_INT, 0);
...
async function loadObj(fn, gl, meshProgramInfo, transforms) {
  ...
  const parts = obj.geometries.map(({ material, data }) => {
    ...
    const vertexArrayInfo = transforms ? twgl.createVertexArrayInfo(gl, meshProgramInfo, bufferInfo) : null;
...
export { BACKWARD, Cam, fetchText, FORWARD, LEFT, loadObj, Mesh, MeshHelper, RIGHT };
